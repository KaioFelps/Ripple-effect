...
var buttons = document.querySelectorAll("button.ripple-button");
function getIsRightButtonPressed(event) {
  if ("which" in event)
    // Gecko (Firefox), WebKit (Safari/Chrome) & Opera
    return event.which === 3;else if ("button" in event)
    // IE, Opera
    return event.button === 2;
}
function removeRippleFromNode(_a) {
  var node = _a.node,
    ripple = _a.ripple;
...
function handleClickRippleSpan(Event) {
  var isRightButton = getIsRightButtonPressed(Event);
  if (isRightButton) {
  ...
  setTimeout(function () {
    return Event.target.classList.remove("click-event-flag");
  }, 10);
  var cursorPositionY = Event.clientY;
  var cursorPositionX = Event.clientX;
  var buttonCurrentColor = window.getComputedStyle(Event.target).color;
  var _a = Event.target.getBoundingClientRect(),
    buttonPositionY = _a.y,
    buttonPositionX = _a.x;
  var ripplePositionY = cursorPositionY - buttonPositionY;
  var ripplePositionX = cursorPositionX - buttonPositionX;
  var rippleSpan = document.createElement("span");
  rippleSpan.classList.add("ripple-span", "active-ripple-span", "click-ripple-span");
...
function handleFocusRippleSpan(Event) {
  ...
  Event.target.appendChild(rippleSpan);
  ["focusout", "blur", "mouseout", "mousedown"].forEach(function (event) {
    Event.target.addEventListener(event, function () {
      removeRippleFromNode({
        node: Event.target,
        ripple: rippleSpan
...
function handleInsetToast(e) {
  var toastContent = e.target.getAttribute("data-toast-content");
  ...
  setTimeout(function () {
    ...
    if (document.querySelector("#portal").childNodes.length === 0) {
      document.querySelector("html").removeChild(document.querySelector("#portal"));
    }
  }, 4000);
}
buttons.forEach(function (button) {
  return button.addEventListener("mousedown", handleClickRippleSpan);
});
buttons.forEach(function (button) {
...
buttons.forEach(function (button) {
  return button.addEventListener("focus", handleFocusRippleSpan);
});
document.querySelectorAll(".toast-cta").forEach(function (botao) {
